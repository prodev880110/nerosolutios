...
const ImportContactsService = async (companyId) => {
    ...
    if (phoneContacts && wbot.type === "legacy") {
        await Promise.all(Object.values(phoneContacts).map(async ({ id, name }) => {
            if (id === "status@broadcast" || id.split("@")[1] === "g.us")
                return;
            ...
            if (numberExists)
                return;
            try {
                await (0, CreateContactService_1.default)({ number, name, companyId });
            }
            catch (error) {
