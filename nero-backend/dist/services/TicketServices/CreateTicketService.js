...
const GetSettingsService_1 = __importDefault(require("../SettingServices/GetSettingsService"));
...
const CreateTicketService = async ({ contactId, status, userId, queueId, companyId }) => {
    ...
    const company_settings = await (0, GetSettingsService_1.default)({ companyId, key });
    if (isGroup && !company_settings.value) {
        throw new AppError_1.default("Can't use GroupMessage. Not allowed.");
    }
    if (!userId) {
        throw new AppError_1.default("Can't use GroupMessage. Not allowed.");
    }
    ...
    await Ticket_1.default.update({ companyId, queueId, userId, status: "open" }, { where: { id } });
