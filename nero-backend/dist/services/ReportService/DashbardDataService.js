    ...
    messagesDaily as (
      select
        (
          select count(distinct "id")
          from "Messages"
          where "companyId" = ? and "createdAt" >= ? and "createdAt" <= ?
        ) "one",
        (
          select count(distinct "id")
          from "Messages"
    ...
    messages as (
      select
        ...
        (
          select count(distinct "id")
          from "Messages"
          where "read" = 'true' and "companyId" = ?
        ) "readMessages",
        (
          select count(distinct "id")
          from "Messages"
          where "read" = 'false' and "companyId" = ?
        ) "unreadMessages"
        
    ),
    schedules as (

          select ss."id", ss."body", ss."sendAt", ss."status"
          from "Schedules" ss
          where "companyId" = ?
          order by ss."sendAt" desc
       
    )
    select
      (select coalesce(jsonb_agg(c.*), '[]')::jsonb from schedules c) schedules,
      (select coalesce(json_agg(a.*), '[]')::jsonb from messages a) messages,
      (select coalesce(json_agg(b.*), '[]')::jsonb from messagesDaily b) messagesDaily;
  `;
    ...
    replacements.push((0, moment_1.default)(d).format("YYYY-MM-DD") + " 00:00:00");
    replacements.push((0, moment_1.default)(d).format("YYYY-MM-DD") + " 23:59:59");
