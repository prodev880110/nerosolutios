...
const CreateUserService = async ({ email, password, name, queueIds = [], companyId, profile = "admin" }) => {
    if (companyId !== undefined) {
        ...
        if (company !== null) {
            ...
            if (usersCount >= company.users_number) {
                throw new AppError_1.default(`Maximum number of users already reached: ${usersCount}`);
            }
        }
    }
    const schema = Yup.object().shape({
        name: Yup.string()
            .required()
            .test("Check-name", "An user with this name already exists.", async (value) => {
            if (!value)
                return false;
            const nameExists = await User_1.default.findOne({
                where: { name: value }
            });
            return !nameExists;
        }),
