...
const CheckCompanyCodeService_1 = __importDefault(require("../services/CompanyService/CheckCompanyCodeService"));
const makeid = (length) => {
    let result = '';
...
const store = async (req, res) => {
    ...
    let company_code = makeid(8);
    let company_code_status = await (0, CheckCompanyCodeService_1.default)({ value: company_code });
    newCompany.code = company_code;
    while (company_code_status == false) {
        company_code = makeid(8);
        newCompany.code = company_code;
        company_code_status = await (0, CheckCompanyCodeService_1.default)({ value: company_code });
        // console.log(company_code_status+":"+company_code);
    }
    newCompany.code = company_code;
