    ...
    try {
        ...
        if (count > 0) {
            schedules.map(async (schedule) => {
                ...
                logger_1.logger.info(`Trigger scheduled for: ${schedule.contact.name}`);
            });
        }
    }
    catch (e) {
...
async function handleSendScheduledMessage(job) {
    ...
    try {
        ...
        Sentry.captureException(e);
        logger_1.logger.info(`Error when trying to query schedule: ${schedule.id}`);
    }
    try {
        ...
        logger_1.logger.info(`Scheduled message sent to: ${schedule.contact.name}`);
...
async function handleVerifyCampaigns(job) {
    ...
    logger_1.logger.info(`Campaigns found: ${campaigns.length}`);
    for (let campaign of campaigns) {
        try {
            const now = (0, moment_1.default)();
            const scheduledAt = (0, moment_1.default)(campaign.scheduledAt);
            const delay = scheduledAt.diff(now, "milliseconds");
            logger_1.logger.info(`Campaign sent to processing queue: Campaign=${campaign.id}, Initial Delay=${delay}`);
...
async function sleep(seconds) {
    logger_1.logger.info(`Sleep of ${seconds} seconds started: ${(0, moment_1.default)().format("HH:mm:ss")}`);
    return new Promise(resolve => {
        setTimeout(() => {
            logger_1.logger.info(`Sleep of ${seconds} seconds done: ${(0, moment_1.default)().format("HH:mm:ss")}`);
            resolve(true);
        ...
        if (campaign) {
            ...
            if ((0, lodash_1.isArray)(contacts)) {
                ...
                for (let contact of contacts) {
                    ...
                    logger_1.logger.info(`Record sent to the firing queue: Campaign=${campaign.id};Contact=${contact.name};delay=${delay}`);
                    index++;
...
async function handleDispatchCampaign(job) {
    try {
        ...
        logger_1.logger.info(`Campaign firing requested: Campaign=${campaignId};Registration=${campaignShippingId}`);
        ...
        logger_1.logger.info(`Campaign sent to: Campaign=${campaignId};Contact=${campaignShipping.contact.name}`);
    }
    catch (err) {
        Sentry.captureException(err);
...
async function handleLoginStatus(job) {
    ...
    for (let item of users) {
        try {
            ...
            logger_1.logger.info(`User taken offline: ${item.id}`);
        }
        catch (e) {
            Sentry.captureException(e);
        }
...
async function startQueueProcess() {
    logger_1.logger.info("Starting queue processing");
